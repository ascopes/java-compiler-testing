...
class StandardCompilationTest {
  ...
  @ParameterizedTest
  void isWarningsAsErrorsIsExpectedValue(boolean warningsAsErors) {
    // Given
    var compilation = someBuilder()
        .warningsAsErrors(warningsAsErors)
        .build();

    // Then
    then(compilation.isWarningsAsErrors()).isEqualTo(warningsAsErors);
  }

  @DisplayName("isSuccessful() is expected value")
  @ValueSource(booleans = {true, false})
  @ParameterizedTest
  void isSuccessfulIsExpectedFalue(boolean successful) {
    // Given
    var compilation = someBuilder()
        .success(successful)
        .build();
